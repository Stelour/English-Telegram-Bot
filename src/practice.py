...
async def handle_practice_choice(update: Update, context: CallbackContext):
    ...
    if chosen_translation == word_data['translation']:
        ...
        if word_data['practice_count'] <= 0:
            ...
            response_text = f"Отлично! Правильный перевод для '{word_to_practice}'. Слово удалено из списка."
        else:
            response_text = f"Отлично! Правильный перевод для '{word_to_practice}'. Его нужно практиковать ещё {word_data['practice_count']} раз."
